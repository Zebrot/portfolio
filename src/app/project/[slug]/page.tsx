...
]|order(publishedAt desc)[0...12]{"slug" : slug.current}`;
...
export default async function Single({
  ...
  const imageUrl = project.thumbnail
    ? getImageUrl(project.thumbnail)?.width(1240).height(1240).url()
    : null;
  ...
  return (
    <div className="container mx-auto min-h-screen max-w-3xl py-8 flex flex-col gap-10 rounded-lg">
      ...
      <div className="">
        ...
        <p className="text-(--lightgrey) my-0 text-[30px]" >étude de cas</p>
      </div>
      <div>
        <div className="rounded-4xl overflow-hidden w-full h-[700px] relative group mb-3">
          <Link href={`/#portfolio`}>
              <Image src={imageUrl?? '/default.png'} alt={project.title} width={1240} height={1240} 
                className="min-h-full min-w-full transition duration-200 group-hover:scale-105 group-hover:blur-xs"
              />
              <div className="opacity-0 group-hover:opacity-100 transition duration-200 absolute 
        ...
        <div className="flex justify-between gap-5">
          ...
          <div className="flex-1 flex justify-end"> 
            <div className="rounded-2xl bg-(--foreground) text-right px-8 py-2 w-fit shadow-xs/30">
